...
@retry_v2(max_attempts=5, delay=10, error_code='AccessDenied')
...
def caller_identity(profile=None):
    """Gets account number from caller identity
    ...
    Returns:
        dict: Returns standard AWS dictionary with account id
    """
    logger.info(f"Getting Account Id")
    sts_client = boto3_client(service='sts', profile=profile)
